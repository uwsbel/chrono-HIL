...
double terrainWidth = 30.0;  // size in Y direction
...
double render_step_size = 1.0 / 30;
...
int main(int argc, char *argv[]) {
  ...
  // Enable moving patch feature (multiple patches around each
  // wheel)
  for (auto &axle : my_hmmwv.GetVehicle().GetAxles()) {
    terrain.AddMovingPatch(axle->m_wheels[0]->GetSpindle(), ChVector<>(0, 0, 0),
                           ChVector<>(1, 0.5, 1));
    terrain.AddMovingPatch(axle->m_wheels[1]->GetSpindle(), ChVector<>(0, 0, 0),
                           ChVector<>(1, 0.5, 1));
  }

  terrain.SetTexture(vehicle::GetDataFile("terrain/textures/grass.jpg"), 80,
                     16);
  ...
  // Simulation loop
  // ---------------
  std::cout << "Total vehicle mass: " << my_hmmwv.GetVehicle().GetMass()
            << std::endl;
  ...
  while (vis->Run()) {
    ...
    driver_inputs.m_steering = driver_inputs.m_steering * -1;
