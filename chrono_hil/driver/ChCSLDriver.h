...
public:
  ...
  /// Set the increment in throttle input for each recorded keypress (default
  /// 1/50).
  void SetThrottleDelta(double delta);

  /// Set the increment in steering input for each recorded keypress (default
  /// 1/50).
  void SetSteeringDelta(double delta);

  /// Set the increment in braking input for each recorded keypress (default
  /// 1/50).
  ...
  void SetGains(double steering_gain, double throttle_gain,
                double braking_gain);
...
protected:
  ...
  std::shared_ptr<ChDataDriver>
      m_data_driver; ///< embedded data driver (for playback)
};
