...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
ChCSLDriver::ChCSLDriver(ChVehicle &vehicle)
    : ChDriver(vehicle), m_steering_target(0), m_throttle_target(0),
      m_braking_target(0), m_stepsize(1e-3), m_steering_delta(1.0 / 50),
      m_throttle_delta(1.0 / 50), m_braking_delta(1.0 / 50),
      m_steering_gain(4.0), m_throttle_gain(4.0), m_braking_gain(4.0),
      m_mode(KEYBOARD), m_joystick(std::make_shared<Joystick>()) {
  m_dT = 0;
...
bool ChCSLDriver::Sample() {
  ...
  //         case KEY_KEY_A:
  //             if (m_mode == KEYBOARD)
  //                 m_steering_target = ChClamp(m_steering_target -
  //                 m_steering_delta, -1.0, +1.0);
  //             return true;
  //         case KEY_KEY_D:
  //             if (m_mode == KEYBOARD)
  //                 m_steering_target = ChClamp(m_steering_target +
  ...
  //         case KEY_KEY_W:
  //             if (m_mode == KEYBOARD) {
  //                 m_throttle_target = ChClamp(m_throttle_target +
  //                 m_throttle_delta, 0.0, +1.0); if (m_throttle_target > 0)
  //                     m_braking_target = ChClamp(m_braking_target -
  //                     m_braking_delta * 3, 0.0, +1.0);
  //             }
  //             return true;
  //         case KEY_KEY_S:
  //             if (m_mode == KEYBOARD) {
  //                 m_throttle_target = ChClamp(m_throttle_target -
  //                 m_throttle_delta * 3, 0.0, +1.0); if (m_throttle_target <=
  //                 0)
  //                     m_braking_target = ChClamp(m_braking_target +
  //                     m_braking_delta, 0.0, +1.0);
  //             }
  //             return true;
  //         default:
  //             break;
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
void ChCSLDriver::SetThrottleDelta(double delta) { m_throttle_delta = delta; }

void ChCSLDriver::SetSteeringDelta(double delta) { m_steering_delta = delta; }

void ChCSLDriver::SetBrakingDelta(double delta) { m_braking_delta = delta; }

void ChCSLDriver::SetGains(double steering_gain, double throttle_gain,
                           double braking_gain) {
...
void ChCSLDriver::SetInputDataFile(const std::string &filename) {
  // Embed a DataDriver.
  m_data_driver =
      chrono_types::make_shared<ChDataDriver>(m_vehicle, filename, false);
}

void ChCSLDriver::SetInputMode(InputMode mode) {
  switch (mode) {
  case LOCK:
    m_mode = LOCK;
    break;
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
void ChCSLDriver::Synchronize(double time) {
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
std::string ChCSLDriver::GetInputModeAsString() const {
  switch (m_mode) {
  case LOCK:
