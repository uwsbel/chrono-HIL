...
//
// =============================================================================
// Authors: Asher Elmquist, Jason Zhou
// =============================================================================
//
...
int main(int argc, char *argv[]) {
  ...
  auto mmesh = ChTriangleMeshConnected::CreateFromWavefrontFile(
      CHRONO_DATA_DIR + std::string("vehicle/audi/audi_chassis.obj"), false,
      true);
  mmesh->Transform(ChVector<>(0, 0, 0),
  ...
  auto floor =
      chrono_types::make_shared<ChBodyEasyBox>(20, 20, .1, 1000, true, false);
  floor->SetPos({0, 0, -1});
  ...
  auto box_body = chrono_types::make_shared<ChBodyEasyBox>(1.0, 1.0, 1.0, 1000,
                                                           true, false);
  ...
  auto sphere_body =
      chrono_types::make_shared<ChBodyEasySphere>(.5, 1000, true, false);
  ...
  auto cyl_body =
      chrono_types::make_shared<ChBodyEasyCylinder>(.25, 1, 1000, true, false);
  ...
  auto ground_body =
      chrono_types::make_shared<ChBodyEasyBox>(1, 1, 1, 1000, false, false);
  ...
  manager->scene->AddPointLight({100, 100, 100},
                                {intensity, intensity, intensity}, 500);
  manager->scene->SetAmbientLight({.1, .1, .1});
  Background b;
  b.mode = BackgroundMode::ENVIRONMENT_MAP;
  b.env_tex = CHRONO_DATA_DIR + std::string("sensor/textures/quarry_01_4k.hdr");
  manager->scene->SetBackground(b);

  // ------------------------------------------------
  // Create a camera and add it to the sensor manager
  // ------------------------------------------------
  chrono::ChFrame<double> offset_pose1({-8, 0, 2},
                                       Q_from_AngAxis(.2, {0, 1, 0}));
  ...
  case CONST_NORMAL:
    cam->PushFilter(
        chrono_types::make_shared<ChFilterCameraNoiseConstNormal>(0.f, .0004f));
    break;
  case PIXEL_DEPENDENT:
    cam->PushFilter(
        chrono_types::make_shared<ChFilterCameraNoisePixDep>(.0004f, .0004f));
    break;
  case NONE:
  ...
  cam->PushFilter(chrono_types::make_shared<ChFilterImageResize>(
  ...
  chrono::ChFrame<double> offset_pose2({5, 0, 0},
                                       Q_from_AngAxis(CH_C_PI, {0, 0, 1}));
  ...
  if (vis)
    cam2->PushFilter(chrono_types::make_shared<ChFilterVisualize>(
        640, 360, "Whitted Ray Tracing"));

  // Save the antialiased image
  if (save)
    cam2->PushFilter(
        chrono_types::make_shared<ChFilterSave>(out_dir + "antialiased/"));
  ...
  if (vis)
    seg->PushFilter(chrono_types::make_shared<ChFilterVisualize>(
        640, 360, "Semantic Segmentation"));

  // Save the semantic mask
  if (save)
    seg->PushFilter(
        chrono_types::make_shared<ChFilterSave>(out_dir + "segmentation/"));
  ...
  if (std::shared_ptr<ChVisualShape> visual_asset =
          std::dynamic_pointer_cast<ChVisualShape>(trimesh_shape)) {
  ...
  // Each camera begins on opposite sides of the object, but rotate at the same
  // speed
  ...
  while (ch_time < end_time) {
    ...
    cam->SetOffsetPose(chrono::ChFrame<double>(
        {orbit_radius * cos(ch_time * orbit_rate),
         orbit_radius * sin(ch_time * orbit_rate), 2},
    ...
    // if (rgba8_ptr->Buffer) {
    //     std::cout << "RGBA8 buffer recieved from cam. Camera resolution: " <<
    //     rgba8_ptr->Width << "x"
    //               << rgba8_ptr->Height << ", frame= " <<
    //               rgba8_ptr->LaunchedCount << ", t=" << rgba8_ptr->TimeStamp
    //               << std::endl
    //               << std::endl;
    // }
    //
    ...
    //         }
    //     }
    //     std::cout << "Average gray value: " << int(running_total) /
    //     double(height * width) << std::endl
    //               << std::endl;
    // }
    //
    ...
    //     std::cout << "First Pixel: [ " << unsigned(first_pixel.R) << ", " <<
    //     unsigned(first_pixel.G) << ", "
    //               << unsigned(first_pixel.B) << ", " <<
    //               unsigned(first_pixel.A) << " ]" << std::endl
    //               << std::endl;
    //
    ...
    //     std::cout << "Last Pixel: [ " << unsigned(last_pixel.R) << ", " <<
    //     unsigned(last_pixel.G) << ", "
